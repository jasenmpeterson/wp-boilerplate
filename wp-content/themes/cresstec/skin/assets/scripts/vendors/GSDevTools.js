...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
    ...
    _gsScope._gsDefine("GSDevTools", ["TweenLite", "core.Animation", "core.SimpleTimeline", "TimelineLite", "utils.Draggable", "plugins.CSSPlugin"], function(TweenLite, Animation, SimpleTimeline, TimelineLite, Draggable) {
        ...
        var _doc = document,
            ...
            _getClippedDuration = function(animation) {
                var max = 0,
                    t = animation._first;
                if (!t) {
            ...
            GSDevTools = function(vars) {
                ...
                var _self = this,
                    ...
                    updateProgress = function(force) {
                        ...
                        var p = (linkedAnimation.progress() * 100) || 0;
                        if (p > 100) {
                            p = 100;
                        }
                        if (p >= outProgress) {
                            if (loopEnabled && !linkedAnimation.paused() && !progressDrag.isDragging) {
                                p = inProgress;
                                linkedAnimation.progress(p / 100).resume();
                            } else {
                                if (p !== outProgress) {
                                    p = outProgress;
                    ...
                    loop = function(value) {
                        ...
                        if (loopEnabled) {
                            loopAnimation.play();
                            if (linkedAnimation.progress() >= outProgress / 100) {
                                linkedAnimation.progress(inProgress / 100).resume();
                            }
                        } else {
                    ...
                    animation = function(anim) {
                        var ts = 1,
                            tl, startTime, endTime, maxDuration;
                        if (!arguments.length) {
                    ...
                    onChangeTimeScale = function(e) {
                        ...
                        if (!paused) {
                            if (linkedAnimation.progress() >= outProgress / 100) {
                                linkedAnimation.progress(inProgress / 100).pause();
                            } else {
                                linkedAnimation.pause();
                            }
                            TweenLite.delayedCall(0.01, function() {
                ...
                if (vars.keyboard !== false) {
                    if (_keyboardInstance && vars.keyboard) {
                        ...
                        _addListener(_docElement, "keydown", function(e) { //window.parent allows things to work inside of an iframe, like on codepen.
